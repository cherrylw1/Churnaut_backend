  ...
  if (isAuthenticated === null) {
    return (
      <div className="min-h-screen bg-[#080B0F] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-xs font-mono text-gray-500 uppercase tracking-widest">Checking Decryption Keys...</p>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-[#080B0F] flex flex-col items-center justify-center p-4 relative overflow-hidden select-none">
        ...
        <div className="w-full max-w-[420px] bg-[#0c0f16]/95 border border-[#1e293b] rounded-xl shadow-[0_0_50px_rgba(99,102,241,0.06)] p-8 relative z-10 transition-all">
          <div className="flex flex-col items-center text-center space-y-4 mb-8">
            <div className="w-12 h-12 rounded-lg bg-[#6366f1]/10 border border-[#6366f1]/25 flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-gray-500 mt-1">Decrypt codebase & live RevOps intelligence console.</p>
            </div>
          </div>

          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-[10px] font-mono text-gray-400 uppercase tracking-widest block">Account ID</label>
              <input
                type="text"
                value={loginAccountId}
                onChange={e => setLoginAccountId(e.target.value)}
                placeholder="e.g. FOUNDER_CLIENT_ID"
                className="w-full bg-[#080B0F] border border-[#1e293b] focus:border-[#6366f1]/50 rounded-[8px] px-3.5 py-2.5 text-sm font-mono text-white placeholder:text-gray-700 outline-none transition-all"
                disabled={loginLoading}
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[10px] font-mono text-gray-400 uppercase tracking-widest block">Founder Passkey</label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[#080B0F] border border-[#1e293b] focus:border-[#6366f1]/50 rounded-[8px] px-3.5 py-2.5 pl-10 text-sm font-mono text-white placeholder:text-gray-700 outline-none transition-all"
                  disabled={loginLoading}
                />
                <KeyRound className="w-4 h-4 text-gray-600 absolute left-3.5 top-1/2 -translate-y-1/2" />
              </div>
            </div>

            {loginError && (
              <div className="flex items-center gap-2 text-xs font-mono text-red-500 bg-red-950/20 border border-red-900/30 p-2.5 rounded-[6px]">
            ...
            <button
              type="submit"
              disabled={loginLoading}
              className="w-full bg-[#6366f1] hover:bg-[#5053e1] disabled:opacity-50 text-white font-mono text-xs font-bold uppercase tracking-widest py-3 rounded-[8px] transition-all active:scale-[0.98] flex items-center justify-center gap-2"
            >
              {loginLoading ? (
                <>
    ...
    <div className="min-h-screen bg-[#080B0F] flex">
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-0'} flex-shrink-0 transition-all duration-200 overflow-hidden border-r border-[#1e1e2e] flex flex-col`}>
        ...
        <div className="p-3 border-t border-[#1e1e2e] flex-shrink-0 flex items-center justify-between">
          ...
          <button onClick={handleLogout} className="flex items-center gap-1 text-[10px] font-mono text-red-500 hover:text-red-400 uppercase tracking-wider transition-colors active:scale-95">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {messages.map(msg => (
            <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.role === 'assistant' && (
                <div className="w-7 h-7 rounded-full bg-[#6366f1]/10 border border-[#6366f1]/25 flex items-center justify-center flex-shrink-0 mt-1">
