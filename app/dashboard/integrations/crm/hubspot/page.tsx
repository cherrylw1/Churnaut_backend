  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({ type: 'success', text: 'HubSpot CRM connection successfully removed.' });
      } else {
        setMessage({ type: 'error', text: 'Failed to disconnect HubSpot CRM. Please try again.' });
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="text-center py-12 text-[var(--text-muted)] font-mono text-sm uppercase tracking-widest bg-[var(--bg-base)] text-[var(--text-primary)] min-h-screen">
        RETRIEVING HUBSPOT CRM STATUS...
      </div>
    );
  }

  const isConnected = status?.connected && status.crm_type === 'hubspot';
      ...
      <div className="border-b border-[var(--border-subtle)] pb-5 flex justify-between items-end">
        <div>
          <div className="flex items-center space-x-2 text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest mb-1.5">
            ...
            <span>/</span>
            <span className="text-[var(--text-secondary)]">HUBSPOT</span>
          </div>
          <h1 className="text-xl font-bold tracking-wider font-mono uppercase text-white">
            HUBSPOT CRM
          </h1>
          <p className="text-xs font-mono text-[var(--text-secondary)] mt-1">
            Map HubSpot lifecycle stages, synchronize pipeline deals, and assign representative owners dynamically.
          </p>
        </div>
        <div>
          <Link
