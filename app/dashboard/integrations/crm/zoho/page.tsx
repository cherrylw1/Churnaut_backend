...
export default function ZohoSettingsPage() {
  ...
  const handleDisconnect = async () => {
    if (!confirm('Are you sure you want to disconnect Zoho CRM? This will remove all synchronization tokens.')) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({ type: 'success', text: 'Zoho CRM connection successfully removed.' });
      } else {
        setMessage({ type: 'error', text: 'Failed to disconnect Zoho CRM. Please try again.' });
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="text-center py-12 text-gray-500 font-mono text-sm uppercase tracking-widest bg-[var(--bg-base)] text-[var(--text-primary)] min-h-screen">
        RETRIEVING ZOHO CRM STATUS...
      </div>
    );
  }

  const isConnected = status?.connected && status.crm_type === 'zoho';
      ...
      <div className="border-b border-[var(--border-subtle)] pb-5 flex justify-between items-end">
        <div>
          <div className="flex items-center space-x-2 text-[10px] font-mono text-gray-500 uppercase tracking-widest mb-1.5">
            ...
            <span>/</span>
            <span className="text-gray-300">ZOHO</span>
          </div>
          <h1 className="text-xl font-bold tracking-wider font-mono uppercase text-white">
            ZOHO CRM
          </h1>
          <p className="text-xs font-mono text-gray-400 mt-1">
            Synchronize Zoho CRM contacts, leads, and deal stages to drive real-time personalization routing.
          </p>
        </div>
        <div>
          <Link
          ...
          <div className="border border-[var(--border-subtle)]/60 bg-[#080B0F] rounded p-4 font-mono text-xs text-gray-400 space-y-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 leading-relaxed">
              <div>
                ...
                <span className="text-white uppercase">ZOHO CRM</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="text-indigo-400 text-[10px] leading-tight">
                  deals.read, contacts.read, leads.read, users.read
                </span>
              </div>
              <div>
