...
export default function CrmIndexPage() {
  ...
  const [unsupportedCallback, setUnsupportedCallback] = useState(false);
  ...
  useEffect(() => {
    ...
    if (connected === 'hubspot') setMessage({ type: 'success', text: 'Successfully connected to HubSpot CRM!' });
    if (error) {
      setUnsupportedCallback(error === 'unsupported_provider');
      ...
      if (error === 'unsupported_provider') errorText = 'Native OAuth is not available for this provider. Use webhook configuration instead.';
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Signal Field · CRM capability" title="CRM integrations" description="Connect the systems that turn account signals into useful routing context." actions={<Link href="/dashboard/integrations" className="dashboard-button-secondary">← CONNECTION MAP</Link>} />
    {message && <div role={message.type === 'error' ? 'alert' : 'status'} aria-live="polite" className={`dashboard-surface px-4 py-3 flex items-start justify-between gap-4 ${message.type === 'error' ? 'border-[var(--red)]/40' : 'border-[var(--green)]/40'}`}><div className="flex flex-wrap items-center gap-3"><p className="text-sm text-[var(--text-secondary)]">{message.text}</p>{unsupportedCallback && <Link href="/dashboard/integrations/webhooks" className="dashboard-button-secondary">OPEN WEBHOOKS →</Link>}</div><button type="button" aria-label="Dismiss integration message" onClick={() => setMessage(null)} className="text-xs text-[var(--text-muted)] hover:text-[var(--text-primary)]">DISMISS</button></div>}
    <Surface>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {providers.map((provider) => {
          ...
          const nativeStatus = loading ? 'Loading' : statusError ? 'Status unavailable' : connected ? 'Connected' : 'Disconnected';
          const body = <div className="h-full rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--bg-elevated)]/55 p-5 flex flex-col gap-5 transition-colors hover:border-[var(--border-strong)]"><div className="flex items-start justify-between gap-3"><h2 className="font-mono text-sm font-semibold text-[var(--text-primary)]">{provider.name}</h2><StatusBadge tone={connected ? 'success' : statusError && provider.kind === 'native' ? 'danger' : provider.kind === 'soon' ? 'neutral' : provider.kind === 'webhook' ? 'warning' : 'neutral'}>{connected ? 'Connected' : provider.kind === 'soon' ? 'Coming Soon' : provider.kind === 'webhook' ? 'Webhook Only' : nativeStatus}</StatusBadge></div><p className="text-sm leading-6 text-[var(--text-secondary)] flex-1">{provider.description}</p>{provider.kind === 'native' && href ? <Link href={href} className="dashboard-button-secondary w-full justify-center">MANAGE →</Link> : provider.kind === 'soon' ? <div className="grid gap-2 sm:grid-cols-2"><button type="button" disabled aria-disabled="true" className="dashboard-button-secondary w-full justify-center opacity-50 cursor-not-allowed">CONNECT →</button>{href && <Link href={href} className="dashboard-button-secondary w-full justify-center">VIEW DETAILS →</Link>}</div> : <span className="rounded-[var(--radius-sm)] border border-[var(--border-subtle)] px-3 py-2 text-center text-[10px] font-mono uppercase tracking-wide text-[var(--text-muted)]">WEBHOOK ONLY — ADAPTER PENDING</span>}</div>;
