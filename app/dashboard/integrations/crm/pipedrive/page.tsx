...
export default function PipedriveSettingsPage() {
  ...
  const handleDisconnect = async () => {
    if (!confirm('Are you sure you want to disconnect Pipedrive CRM? This will remove all synchronization tokens.')) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({ type: 'success', text: 'Pipedrive CRM connection successfully removed.' });
      } else {
        setMessage({ type: 'error', text: 'Failed to disconnect Pipedrive CRM. Please try again.' });
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="text-center py-12 text-[var(--text-muted)] font-mono text-sm uppercase tracking-widest bg-[var(--bg-base)] text-[var(--text-primary)] min-h-screen">
        RETRIEVING PIPEDRIVE CRM STATUS...
      </div>
    );
  }

  const isConnected = status?.connected && status.crm_type === 'pipedrive';
      ...
      <div className="border-b border-[var(--border-subtle)] pb-5 flex justify-between items-end">
        <div>
          <div className="flex items-center space-x-2 text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest mb-1.5">
            ...
            <span>/</span>
            <span className="text-[var(--text-secondary)]">PIPEDRIVE</span>
          </div>
          <h1 className="text-xl font-bold tracking-wider font-mono uppercase text-white">
            PIPEDRIVE CRM
          </h1>
          <p className="text-xs font-mono text-[var(--text-secondary)] mt-1">
            Map Pipedrive pipeline deals, synchronize contacts, and assign representative owners dynamically.
          </p>
        </div>
        <div>
          <Link
