...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    fetch('/api/oauth/crm').then((res) => res.ok ? res.json() : Promise.reject(new Error('status'))).then(setCrmStatus).catch(() => setCrmError(true)).finally(() => setCrmLoading(false));
    fetch('/api/oauth/calendly/status').then((res) => res.ok ? res.json() : Promise.reject(new Error('status'))).then(setCalendlyStatus).catch(() => setCalendlyError(true)).finally(() => setCalendlyLoading(false));
    fetch('/api/client').then((res) => res.ok ? res.json() : Promise.reject(new Error('plan'))).then((data) => data.client?.plan && setPlan(data.client.plan)).catch(() => setPlanError(true)).finally(() => setPlanLoading(false));
  }, []);
