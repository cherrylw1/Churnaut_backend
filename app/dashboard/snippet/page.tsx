...
export default function SnippetPage() {
  ...
  if (loading) {
    return <div className="text-center py-12 text-[var(--text-muted)] font-mono text-sm">RETRIEVING SNIPPET CONFIGURATIONS...</div>;
  }
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="border border-[var(--border-subtle)] bg-[var(--bg-surface)] rounded-lg p-6 space-y-4">
        <div className="flex items-center space-x-3">
          <span className="text-xs font-mono font-bold bg-[var(--border-subtle)] border border-[var(--border-subtle)] text-[var(--accent)] px-2 py-0.5 rounded">
            STEP 1
          </span>
        ...
        <div className="relative group border border-[var(--border-subtle)] bg-[#1e1e2e] rounded-lg p-4 font-mono text-xs overflow-x-auto text-[#e2e8f0]" style={{ backgroundColor: '#1e1e2e', color: '#e2e8f0' }}>
          ...
          <button
            ...
            className="absolute top-3 right-3 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white font-mono text-[10px] py-1.5 px-3 rounded transition-all active:scale-[0.98]"
          >
            {copied ? 'COPIED!' : 'COPY'}
          </button>
        </div>
      ...
      <div className="border border-[var(--border-subtle)] bg-[var(--bg-surface)] rounded-lg p-6 space-y-4">
        ...
        <p className="text-xs font-mono text-[var(--text-secondary)]">
          {"Add the class "}
          <span className="text-[var(--accent)] font-bold bg-[var(--border-subtle)] px-1.5 py-0.5 rounded">sr-target</span>
        ...
        <div className="border border-[var(--border-subtle)] bg-[#1e1e2e] rounded-lg p-4 font-mono text-xs text-[#e2e8f0]" style={{ backgroundColor: '#1e1e2e', color: '#e2e8f0' }}>
          <span className="text-[var(--text-muted)] block mb-2">&lt;!-- Example element swaps --&gt;</span>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-2">
          ...
          {status && (
            <div className="flex-1 flex justify-end">
              {status.active ? (
                <div className="flex items-center space-x-3 border border-[var(--green)]/30 bg-[var(--green)]/10 text-[var(--green)] p-3 rounded-lg font-mono text-xs">
                  ...
                  <div>
                    <span className="font-bold block text-[var(--green)]">CONNECTION CONFIRMED</span>
                ...
                <div className="border border-[var(--amber)]/30 bg-[var(--amber)]/10 text-[var(--amber)] p-4 rounded-lg font-mono text-xs space-y-2 max-w-md w-full">
                  <div className="flex items-center space-x-2">
                    <span className="h-2 w-2 rounded-full bg-[var(--amber)]"></span>
                    <span className="font-bold">WAITING FOR PINGS</span>
                  </div>
                  <div className="text-[10px] text-[var(--text-secondary)] leading-relaxed space-y-1">
                    <span className="block font-bold text-[var(--text-primary)]">Troubleshooting Tips:</span>
      ...
      <div className="space-y-4">
        <h2 className="text-xs font-mono font-bold text-[var(--accent)] uppercase tracking-widest bg-[var(--border-subtle)]/40 py-1.5 px-3 rounded border border-[var(--border-subtle)] inline-block">
