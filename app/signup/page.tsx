  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSignup} className="space-y-4" aria-busy={loading}>
        ...
        <button type="submit" disabled={loading} aria-busy={loading} className="auth-submit-button">{loading ? 'INITIALIZING WORKSPACE…' : <>GET STARTED <ArrowRight className="ml-auto h-4 w-4" aria-hidden="true" /></>}</button>
      </form>
    </AuthShell>
  );
}
