  ...
  return (
    <div className="mt-4 space-y-1" aria-label="Recent activity feed">
      {events.map((event, index) => (
        <div key={`${event.created_at}-${index}`} className="dashboard-signal-row group flex items-start gap-3 border border-transparent px-3 py-3 transition-colors hover:border-[var(--border-subtle)] hover:bg-[var(--bg-elevated)]">
          ...
          <div className="min-w-0 flex-1">
            <p className="dashboard-wrap-anywhere text-sm font-medium text-[var(--text-primary)]">{event.event_type}</p>
            <p className="dashboard-wrap-anywhere mt-1 text-[11px] font-mono uppercase tracking-[0.06em] text-[var(--text-muted)]">{event.signal_type || 'N/A'}</p>
          </div>
