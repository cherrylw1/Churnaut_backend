...
export function PressureInstrument({ score, status, value }: { score: number; status: PressureStatus; value?: ReactNode }) {
  ...
  return (
    <Surface className="dashboard-pressure-instrument relative overflow-hidden p-5 md:p-6" aria-label={`Pipeline pressure ${score}, ${status}`}>
      ...
      <div className="mt-8 flex items-end gap-4">
        <span className="font-mono text-5xl font-semibold leading-none tracking-[-0.08em] text-[var(--text-primary)] tabular-nums">{value ?? score}</span>
        <div className="pb-1">
          <StatusBadge tone={toneForStatus(status)}>{status}</StatusBadge>
          <p className="mt-2 flex items-center gap-1 text-[10px] font-mono uppercase tracking-[0.08em] text-[var(--text-muted)]">
            Current readout
          </p>
        </div>
      </div>
      <div className="mt-6" role="img" aria-label={`${score} out of 100 pipeline pressure`}>
        <div className="h-2 overflow-hidden rounded-full border border-[var(--border-subtle)] bg-[var(--room-panel-muted)]">
        ...
        <div className="mt-2 flex justify-between text-[10px] font-mono uppercase tracking-[0.08em] text-[var(--text-muted)]" aria-hidden="true">
