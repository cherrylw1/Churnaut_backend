...
function getChangedFiles(): string[] {
  try {
    ...
    const output = execSync('git diff --name-only HEAD~1 HEAD 2>/dev/null || git diff --name-only HEAD 2>/dev/null || echo ""').toString().trim()
    if (!output) return TARGET_FILES // fallback: index everything
    const changed = output.split('\n').filter(Boolean)
    // Only return files that are in our TARGET_FILES list
    return TARGET_FILES.filter(f => changed.includes(f))
  } catch {
    return TARGET_FILES // fallback: index everything
  }
}
...
async function ingestChanged() {
  const filesToIndex = getChangedFiles()

  if (filesToIndex.length === 0) {
    console.log('No tracked files changed — skipping ingestion.')
    process.exit(0)
  }

  console.log(`Re-indexing ${filesToIndex.length} changed file(s)...`)
  ...
  console.log(`\nDone. ${totalChunks} chunks re-indexed. ${skipped} files skipped.`)
}

ingestChanged().catch(console.error)
